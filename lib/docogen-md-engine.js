...
function parse_content(ast_obj_array){
    ...
    // Here comes ListItem
    var type = "",content_element = { data: [] , figure: []};
    ...
    for(var index=0;index < ast_obj_array.children[1].children.length ;index++){
        ...
        else if(ast_obj_array.children[1].children[index].children[0].raw == "figure"){
            content_element.figure = parse_by_block(ast_obj_array.children[1].children[index].children)
        }
        else if(ast_obj_array.children[1].children[index].children[0].raw == "subarticle"){
            content_element["subarticle"] = create_subarticle(ast_obj_array.children[1].children[index].children)
        }
        else{
...
function nested_rec(obj_arr,options){
    ...
    for(var index=0;index<obj_arr[1].children.length;index++){
        ...
        if(obj_arr[1].children[index].children != undefined && obj_arr[1].children[index].children.length > 1){
            // Nested!
            element[options.nested] = nested_rec(obj_arr[1].children[index].children,options)
        }
        arr.push(element);
    }
    return arr
